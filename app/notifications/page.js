...
const BookClubsPage = () => {  
  ...
  return (
    ...
    <div className="min-h-screen bg-white">
        ...
        <section >
          ...
          <div className="flex justify-center items-center flex-wrap space-y-8 p-6"> 
            ...
            <button onClick={() => router.push('/landing')} className={`relative group px-2 py-2 rounded-lg bg-transparent text-gray-500 font-medium overflow-hidden bottom-5 ${header2Font.className}`}>
              ...
              <button onClick={() => router.push('/notifications')} className={`relative group w-full px-4 py-2 rounded-lg bg-transparent text-white font-medium overflow-hidden ${header2Font.className}`}>
                <span className="absolute inset-0 bg-red-200 transition-transform translate-x-full group-hover:translate-x-0 group-hover:rounded-lg group-hover:border-4 group-hover:border-black"></span>
      ...
      <div className={`max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8 ${header2Font.className}`}>
        {/* Notifications Section */}
        <section className="mb-8">
          <div className={`flex items-center gap-2 mb-4 ${headerFont.className}`}>
            <Bell className="w-5 h-5" />
            <h2 className={`text-3xl font-semibold""${header2Font.className}`}>Recent Notifications</h2>
          </div>
